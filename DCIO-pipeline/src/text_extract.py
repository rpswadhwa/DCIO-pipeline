...
def _pdf_has_northern_trust_schedule_layout(pdf_path: str, pages: List[int]) -> bool:
    """Detect Northern Trust's "Schedule of Assets (Held at End of Year)" custody
    format (two-line-per-security, six trailing decimal columns). This layout
    renders with column-like text positioning that Camelot can mistake for a
    ruled table, so it must be routed to the dedicated text parser before
    Camelot ever gets a chance to mis-detect it -- mirroring how the GM/HH1C
    check above is applied before Camelot for the same reason."""
    try:
        with pdfplumber.open(pdf_path) as pdf:
            pages_to_check = [p for p in pages if 1 <= p <= len(pdf.pages)] or range(1, len(pdf.pages) + 1)
            checked = 0
...
_NORTHERN_TRUST_HEADER_MARKERS = (
...
def _pdf_page_has_northern_trust_schedule(text_upper: str) -> bool:
    return all(marker in text_upper for marker in _NORTHERN_TRUST_HEADER_MARKERS)


_NT_BOND_RE = re.compile(r'\d+(?:\.\d+)?%|\bBEO\b|\bMUNICIPAL\b|\bREV\b', re.IGNORECASE)
_NT_CF_PREFIX_RE = re.compile(r'^CF\b', re.IGNORECASE)
_NT_CASH_PREFIX_RE = re.compile(r'^CASH\s+HELD\b', re.IGNORECASE)
_NT_PARTNERSHIP_RE = re.compile(r'\bPARTNERSHIP\b|\bJOINT\s+VENTURE\b|\bL\.?P\.?$|\bFEEDER\s+FUND\b', re.IGNORECASE)


def _classify_northern_trust_other_bucket(desc: str) -> str:
    """Northern Trust's 'Other' section (everything outside 'Value of
    Interest in Registered Investment Companies') mixes bonds, cash sweep
    vehicles, private/collective funds, and partnership interests with no
    per-row type label -- only the security's own name hints at its type.
    Northern Trust prefixes its own collective/commingled fund vehicles
    with "CF " (e.g. "CF BRIDGEWATER PURE ALPHA...", "CF ULLICO..."). Names
    that don't match any of these shapes (e.g. "ALCENTRA EUROPEAN CREDIT
    OPPORTUNITIES FUND") are left unclassified rather than guessed, so they
    fall into the existing blank-asset_type reclassification pipeline
    instead of being mistyped."""
    if _NT_CF_PREFIX_RE.match(desc):
        return 'Common/Collective Trust Fund'
    if _NT_CASH_PREFIX_RE.match(desc):
        return 'Money Market Fund'
    if _NT_PARTNERSHIP_RE.search(desc):
        return 'Joint Venture'
    if _NT_BOND_RE.search(desc):
        return 'Bond'
    return ''


def _extract_northern_trust_schedule_for_pdf(pdf_path: str) -> List[Dict]:
    """Parse Northern Trust's "Schedule of Assets (Held at End of Year)"
    custody-statement layout (footer: "Generated by Northern Trust from
    reviewed periodic data"). Each security occupies two text lines: line 1
    carries the description plus six decimal-valued trailing columns
    (Shares/Par Value, Market Price, Market Value, ERISA Cost, Unrealized
    Security Movements, Unrealized Gain/Loss); line 2 carries only the
    security's internal Asset ID (e.g. "C922040209"), sometimes preceded by
    leftover name-overflow text wrapped from line 1 (e.g.
    "FIDELITY C316146265"), with no dollar amount and no literal
    "CUSIP"/"SEDOL" label anywhere.

    Generic text-based extraction (extract_text_based_investments) cannot
    see the real security rows here at all -- its value patterns only match
    plain digit/comma runs, and every real value on this page ends in a
    decimal (sometimes negative). The bare Asset ID line, having no
    decimal point, was the ONLY thing generic extraction could match --
    producing a phantom row (name "C", value = the ID's own digit suffix
    misread as dollars) for every single security, with zero real holdings
    ever captured. This parser reads the real 3rd trailing numeric column
    (Market Value) directly via _split_trailing_numeric_tokens (which
    requires an explicit decimal point per token, so a bare integer-suffix
    ID line never qualifies as a 6-column data row in the first place).
    """
    all_rows: List[Dict] = []
    current_asset_type = ''
    row_num = 0
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page_idx, page in enumerate(pdf.pages, start=1):
            text = page.extract_text() or ''
            if not _pdf_page_has_northern_trust_schedule(text.upper()):
                continue
            for raw_line in text.split('\n'):
                line = normalize_whitespace(raw_line)
                if not line or line.upper().startswith('GENERATED BY NORTHERN TRUST'):
                    continue
                lead, tail = _split_trailing_numeric_tokens(line)
                if len(tail) != 6:
                    # Not a standard 6-column data row -- a heading, a
                    # region/category subtotal (4 trailing numbers), page
                    # furniture, or an Asset ID continuation line (0
                    # trailing numbers, since a bare ID like "C922040209"
                    # has no decimal point and never matches the numeric
                    # token pattern at all).
                    if not tail:
                        heading = _match_asset_category_text(line)
                        if heading:
                            current_asset_type = heading
                        elif line.strip().upper() == 'OTHER':
                            # Genuinely mixed bucket (bonds, cash, private
                            # funds, partnerships) -- clear the inherited
                            # RIC/Mutual-Fund type rather than let it bleed
                            # onto unrelated rows; per-row classification
                            # below takes over instead.
                            current_asset_type = ''
                    continue
                if lead.lower().startswith('total'):
                    continue
                current_value = tail[2].replace(',', '').strip('()')
                asset_type = current_asset_type or _classify_northern_trust_other_bucket(lead)
                row_num += 1
                all_rows.append({
                    'issuer_name': lead,
                    'investment_description': lead,
                    'asset_type': asset_type,
                    'par_value': tail[0].replace(',', ''),
...
def extract_text_based_investments(pdf_path: str, page_num: int, parser_profile: str = "", inherited_asset_type: str = "") -> List[Dict]:
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        def _value_glued_to_prev_char(s: str, m) -> bool:
            ...
            # not a real name/value column split -- it's the numeric tail of
            # a single unbroken alphanumeric token. Custodians like Northern
            # Trust print each security's internal Asset ID (e.g.
            # "C922040209", or a name-overflow word plus the ID like
            # "FIDELITY C316146265") alone on a continuation line with no
            # dollar amount at all and no literal "CUSIP"/"SEDOL" label (the
            # pre-existing _CUSIP_ARTIFACT_RE guard only catches filers that
            # print that label). Without this check, simple_value_pattern /
            # _TYPE_ANCHORED_VALUE_RE greedily grab the ID's trailing digits
            # as a "value" and leave a stray leading letter (or letter(s))
            # as a garbage issuer name -- producing phantom rows (name "C",
            # value in the hundreds of millions) that don't correspond to
            # any real holding. Real filer rows always have at least one
            # space between the fund name and its value column, so requiring
            # that space here rejects the glued-ID case without affecting
            # genuine matches (verified against the PPG "...Mutual Fund 57"
            # case _TYPE_ANCHORED_VALUE_RE exists for, which still has a
        ...
        for i in range(data_start_idx, len(lines)):
            ...
            if dual_match:
                ...
                if value_match and _value_glued_to_prev_char(stripped, value_match):
                    value_match = None
            else:
                stripped = line
                value_match = None
            ...
            if dual_match:
                ...
                if value_match:
                    ...
                    if value_match and _value_glued_to_prev_char(line, value_match):
                        value_match = None
                    if not value_match:
...
def extract_tables_and_map(
    ...
    # Northern Trust's custody-statement "Schedule of Assets (Held at End of
    # Year)" layout: two-line-per-security rows with six trailing decimal
    # columns. Generic text-value patterns are blind to decimal-formatted
    # values, so this must route to the dedicated parser before Camelot
    # and before falling through to extract_text_based_investments.
    if _pdf_has_northern_trust_schedule_layout(pdf_path, supplemental_pages):
        nt_rows = _extract_northern_trust_schedule_for_pdf(pdf_path)
        if nt_rows:
