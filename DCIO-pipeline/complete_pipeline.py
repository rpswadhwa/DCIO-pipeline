...
for fname in sorted(os.listdir(input_dir)):
    ...
    if supp_nums:
        ...
        extracted_count = sum(len(p.get('mapped_rows', [])) for p in extracted)
        print(f"    Extracted {extracted_count} investment records")
...
with sqlite3.connect(db_path) as conn:
    ...
    if 'fund_name' not in existing_cols:
        ...
        print(f"  Added fund_name column to investments table")
    ...
    # Also create placeholder entries for PDFs that had clean records but no EIN
    stems_with_records = {r.get('pdf_stem') for r in clean_investments if r.get('pdf_stem')}
    for stem in stems_with_records:
        if stem not in plan_info:
            plan_info[stem] = {'ein': None, 'plan_name': stem, 'plan_number': '001', 'sponsor': stem}
    ...
    for pdf_stem, info in plan_info.items():
        ...
        if not sponsor_ein:
            sponsor_ein = f"UNKNOWN_{pdf_stem}"
            print(f"  Warning: No EIN for {pdf_stem}, using synthetic ID: {sponsor_ein}")
        ...
        for row in clean_investments:
            ...
            cur.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                sponsor_ein, page_number, row_id,
                row.get('issuer_name', ''),
                row.get('investment_description', ''),
                row.get('asset_type', ''),
                par_value, cost, current_value, units_or_shares, 1.0,
                row.get('fund_name', '')
            ))
            inserted_count += 1
